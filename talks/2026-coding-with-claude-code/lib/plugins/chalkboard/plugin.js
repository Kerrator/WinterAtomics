...
*/
	...
	var transition = 800;
	// Local patch (stylus-only palm rejection): where the platform reports
	// Touch.touchType (iPadOS Safari: 'stylus' = Apple Pencil, 'direct' =
	// finger/palm), only the stylus inks. Platforms without touchType are
	// unaffected. Disable with chalkboard: { stylusOnly: false }.
	var stylusOnly = true;
	...
	function configure( config ) {
		...
		if ( config.stylusOnly != undefined ) stylusOnly = config.stylusOnly;
 ...
 ******************************************************************/
	...
	// Local patch (stylus-only palm rejection): pick the touch allowed to ink
	// from a TouchList. With stylusOnly, a touch inks only if the platform does
	// not report touchType (non-Apple devices) or reports 'stylus' (the Pencil);
	// 'direct' finger/palm contacts return null and are left to reveal.
	function inkingTouch( touchList ) {
		for ( var i = 0; i < touchList.length; i++ ) {
			var t = touchList[ i ];
			if ( !stylusOnly || t.touchType == undefined || t.touchType == 'stylus' ) return t;
		}
		return null;
	}
...
// TODO: check all touchevents
		canvas.addEventListener( 'touchstart', function ( evt ) {
			var inkTouch = inkingTouch( evt.changedTouches );
			if ( !inkTouch ) return; // palm/finger landed: not an ink event
			evt.preventDefault();
//console.log("Touch start");
		...
		canvas.addEventListener( 'touchmove', function ( evt ) {
			var inkTouch = inkingTouch( evt.touches );
			if ( !inkTouch ) return; // only palm/finger on the glass
			evt.preventDefault();
//console.log("Touch move");
		...
		canvas.addEventListener( 'touchend', function ( evt ) {
			// If a palm/finger lifted while the stylus is still down, keep the stroke.
			if ( !inkingTouch( evt.changedTouches ) && inkingTouch( evt.touches ) ) return;
			evt.preventDefault();
			stopDrawing();
			stopErasing();
		}, false );
